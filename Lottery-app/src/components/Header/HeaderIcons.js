...
import Animated, { concat, EasingNode, Extrapolate, timing, Value } from "react-native-reanimated";
...
function BackButton(props) {
  if (props.icon === "leftArrow") {
    return <Ionicons name="ios-arrow-back" size={scale(30)} style={styles.leftIconPadding} color={props.iconColor} />;
  } else if (props.icon === "menu") {
    return <MaterialIcons name="menu" size={scale(30)} style={styles.leftIconPadding} color={props.iconColor} />;
  } else if (props.icon === "dots") {
    return <MaterialCommunityIcons name="dots-vertical" size={scale(30)} color={props.iconColor} />;
  } else {
    return (
...
function LeftButton(props) {
  ...
  if (props.icon === "back") {
    return (
      <HeaderBackButton
        labelVisible={false}
        backImage={() => BackButton({ iconColor: props.iconColor, icon: "leftArrow" })}
        onPress={() => {
          navigation.goBack();
        }}
      />
    ...
    return (
      <HeaderBackButton
        labelVisible={false}
        backImage={() => BackButton({ iconColor: props.iconColor, icon: "menu" })}
...
function RightButton(props) {
  ...
  if (props.icon === "dots") {
    return (
      <View>
        {password ? (
          <TouchableOpacity activeOpacity={0.7} style={styles.rightContainer} onPress={props.modalVisible}>
          ...
          <HeaderBackButton
            labelVisible={false}
            backImage={() => (
              <View style={styles.rightContainer}>{BackButton({ iconColor: props.textColor, icon: "dots" })}</View>
            )}
